...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    return new Response("gateway-error-pipeline: POST /run to trigger manually, otherwise runs on cron.", {
      status: 200,
    });
  },
  ...
  async scheduled(_controller: ScheduledController, env: Env, ctx: ExecutionContext): Promise<void> {
    ctx.waitUntil(
      (async () => {
        const summary = await runIngestion(env);
        console.log("gateway-error-pipeline run", JSON.stringify(summary));
      })(),
    );
  },
